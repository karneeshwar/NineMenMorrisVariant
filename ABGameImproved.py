...
# Extra: ALPHA-BETA. Program: ABGameImproved.py
# It gets user input for input file name, output file name, and depth for the game
# It uses AlphaBeta pruning algorithm with improved static estimation to compute the best possible midgame-endgame move for the Maximizer (White)
...
if __name__ == '__main__':
    ...
    game_phase = 'midgame-endgame'
    print()

    write_position = []
    for position in input_positions:
        result = ALPHABETA.alphabeta_improved('W', int(input_list[2]), position, game_phase, 0, float('-inf'), float('inf'))
        print('INPUT:')
